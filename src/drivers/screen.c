...
static int current_line_idx =
    0; // Circular index in buffer where we are currently writing
static int current_col = 0;
static int scroll_offset =
    0; // How many lines scrolled UP from the current line
...
static void clear_history_line(int idx) {
  for (int i = 0; i < SCREEN_COLS; i++) {
    history_buffer[idx][i] = (current_color << 8) | ' ';
  }
}
...
void refresh_screen() {
  ...
  if (start_history_total < 0)
    start_history_total = 0;
  ...
  for (int r = 0; r < VISIBLE_ROWS; r++) {
    ...
    if (max_scroll > 0) {
      int handle_r = (VISIBLE_ROWS - 1) -
                     (scroll_offset * (VISIBLE_ROWS - 1) / max_scroll);
      if (r == handle_r) {
...
void scroll_up() {
  ...
  if (scroll_offset < max_scroll &&
      scroll_offset < MAX_HISTORY - VISIBLE_ROWS) {
    scroll_offset++;
    refresh_screen();
    update_cursor();
  }
...
void print_char(char c) {
  if (c == '\b') {
    if (current_col > 0) {
      current_col--;
      history_buffer[current_line_idx][current_col] =
          (current_color << 8) | ' ';
    }
  } else if (c == '\n') {
    current_col = 0;
    ...
    if (current_col < SCREEN_COLS - 1) {
      history_buffer[current_line_idx][current_col] = (current_color << 8) | c;
      current_col++;
    } else {
      // Auto wrap
      print_char('\n');
