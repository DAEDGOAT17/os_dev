...
int fat32_open(const char* path, char mode) {
    ...
    if (cluster == 0 && mode == 'w') {
        ...
        if (!cluster) return -1;
        set_next_cluster(cluster, FAT32_EOC);
        
        // Zero out the newly allocated cluster so text reads terminate properly
        ...
        if (create_entry(fat32_cwd_cluster, path, FAT_ATTR_ARCHIVE, cluster, 0) != 0) return -1;
    }
    ...
    open_files[fd].current_cluster = cluster;
...
int fat32_write(int fd, const void* buf, uint32_t len) {
    ...
    if (bytes_written > 0) {
        uint32_t search_cluster = fat32_cwd_cluster;
        if (search_cluster == 0) search_cluster = bpb.root_cluster;
        
        uint8_t name_8[8], ext_3[3];
        standardize_name(file->name, name_8, ext_3);
        ...
        while (search_cluster != FAT32_EOC && !found) {
            ...
            for (int s = 0; s < bpb.sectors_per_cluster && !found; s++) {
                ...
                for (int i = 0; i < 512 / sizeof(fat32_dir_entry_t); i++) {
                    ...
                    if (entries[i].name[0] == 0xE5) continue;
                    if (memcmp(entries[i].name, name_8, 8) == 0 && memcmp(entries[i].ext, ext_3, 3) == 0) {
                        entries[i].file_size = file->position;
                        ata_write_sectors(lba + s, 1, sector_buf);
                        found = 1;
                        break;
                    }
