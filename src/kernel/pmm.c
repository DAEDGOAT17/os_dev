...
void pmm_init(multiboot_info_t *mbd) {
  ...
  if (mbd->flags & (1 << 6)) {
    ...
    // Fallback: If no memory map provided, assume 128MB is available
    // (This is risky but better than hanging)
    for (uint32_t addr = 0x100000; addr < 0x8000000; addr += PMM_BLOCK_SIZE) {
      pmm_clear_bit(addr / PMM_BLOCK_SIZE);
    }
  }
